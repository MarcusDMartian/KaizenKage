      ...
      <div className="relative z-10 flex flex-col min-h-screen">
        ...
        <header className="hidden md:flex fixed top-4 left-6 right-6 h-18 z-50 items-center justify-between px-6 py-3 rounded-2xl bg-white/60 backdrop-blur-2xl border border-white/60 shadow-lg shadow-indigo-500/5 transition-all">
          ...
          <div className="flex items-center gap-3">
            ...
            <span className="text-xl font-bold text-slate-800 tracking-tight">KaizenHub</span>
          </div>
          ...
          <div className="flex items-center gap-5">
             {/* Notification */}
             <div className="relative cursor-pointer text-slate-500 hover:text-indigo-600 transition-colors p-1.5 rounded-full hover:bg-white/50">
                <Bell size={20} />
             ...
             <div className="flex items-center gap-3">
                 <Link to="/profile" className="flex items-center gap-3 group">
                     <div className="text-right hidden lg:block">
                        <p className="text-sm font-bold text-slate-800 leading-none group-hover:text-indigo-700 transition-colors">{CURRENT_USER.name}</p>
                        <p className="text-xs text-indigo-600 font-bold mt-0.5">{CURRENT_USER.points} pts</p>
                     </div>
                     <div className="relative">
                       <img src={CURRENT_USER.avatar} alt="Avatar" className="w-9 h-9 rounded-full border-2 border-white shadow-sm group-hover:scale-105 transition-transform" />
                       <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-green-500 border-2 border-white rounded-full"></div>
                     </div>
                 </Link>
                 
                 <button className="text-slate-400 hover:text-red-500 transition-colors p-1.5 hover:bg-red-50 rounded-lg" title="Logout">
                    <LogOut size={18} />
                 </button>
             </div>
          </div>
        ...
        <main className="flex-1 flex flex-col min-h-screen relative pb-28 md:pb-8 md:pt-28 md:px-8">
          ...
          {/* Mobile Header - Glass (Keep existing logic) */}
          <header className="md:hidden sticky top-0 z-20 bg-white/70 backdrop-blur-xl border-b border-white/50 px-4 py-3 flex items-center justify-between safe-top shadow-sm mb-4">
            <div className="flex items-center gap-3">
              ...
              <span className="font-bold text-slate-800 text-lg tracking-tight">KaizenHub</span>
            </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <Link to="/profile">
                 <div className="p-0.5 rounded-full bg-gradient-to-tr from-indigo-500 to-purple-600">
                    <img src={CURRENT_USER.avatar} alt="Avatar" className="w-8 h-8 rounded-full border-2 border-white" />
                 </div>
              </Link>
            </div>
          </header>
        ...
        {/* Mobile Bottom Navigation - Floating Glass (Keep existing) */}
